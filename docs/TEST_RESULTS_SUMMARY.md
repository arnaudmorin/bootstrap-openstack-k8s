# Nova Tests Results Summary

## Date: 2026-01-27

## Test Execution

Ran all Nova API tests from `tests/nova/` directory.
...
- Instance ID: `f634e30e-d86d-486d-86fa-7fc24b7459be`
...
- Instance returned to ACTIVE status after reboot
...
1. OAuth2 authentication **works** (token is accepted)
2. Nova tries to make service-to-service calls (to Glance, Neutron, etc.)
3. OAuth2 tokens from Keycloak **cannot** be used for OpenStack service-to-service authentication
4. This causes an `AuthorizationFailure` exception
...
**Status:** ✅ Implementation validated - Dual authentication working correctly
